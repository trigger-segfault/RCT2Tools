...
public class Attraction : ObjectData {
	...
	/** <summary> Constructs the default object. </summary> */
	public Attraction(ObjectDataHeader objectHeader, ChunkHeader chunkHeader)
	...
	public override bool Draw(Graphics g, Point position, int rotation = 0, int corner = 0, int slope = -1, int elevation = 0, int frame = 0) {
		try {
			if (Header.RideType == RideTypes.Stall) {
				graphicsData.PaletteImages[3 + rotation].Draw(g,
					position.X + imageDirectory.Entries[3 + rotation].XOffset,
					position.Y + imageDirectory.Entries[3 + rotation].YOffset,
					Palette.DefaultPalette,
					GraphicsData.ColorRemap1,
					-1,
					-1
				);
				if ((rotation == 0 || rotation == 3) && (Header.TrackType == TrackTypes.Restroom || Header.TrackType == TrackTypes.FirstAid)) {
					graphicsData.PaletteImages[3 + 4 + rotation / 3].Draw(g,
						position.X + imageDirectory.Entries[3 + 4 + rotation / 3].XOffset,
						position.Y + imageDirectory.Entries[3 + 4 + rotation / 3].YOffset,
						Palette.DefaultPalette,
						GraphicsData.ColorRemap1,
						-1,
						-1
	...
	public override bool DrawDialog(Graphics g, Point position, int rotation = 0) {
		try {
			int frame = (imageDirectory.Entries[0].Width == 1 ? 1 : 0);
			g.DrawImage(graphicsData.Images[frame], new Point(
